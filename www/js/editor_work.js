...
var page = function(){
	...
	this.myEvent = function(thislink){
		if (window.getSelection){
			...
			if(obj1.toString() == ''){
				...
				// если задан размер текста,
				if(parent.style.fontSize!=''){
					// alert(parent.style.fontSize+'-parent');
					this.buttons.SelFS.element.value = parent.style.fontSize;
				}
	...
	this.onclick_some = function(objButton,objthis){
		...
		switch(state){
			...
			// выделена строка
			case 1:
				document.execCommand(objButton.command,false,null);
				// alert('1 objButton.command-'+objButton.command);
				break;
			// выбран элемент выделен div, строка не выделена
			case 2: 
				var prop = objButton.cssProp;
				// alert('style-'+objthis.answers.item(0).style+'-');
				var css = objButton.cssVal;
				var anticss = objButton.anticssVal;
				var add = true;
				var elem;
				...
				// проверяем совпадает ли задаваемое свойство с текущим
				if(elem.item(0).style[prop]==css){
					// для textAign и fontSize значение будем менять всегда
					if(prop!='textAlign' && prop!='fontSize'){
						add = false;
					}
				}
				for(var i = 0;i<elem.length;i++){
					if(add){
						// если задаем не размер шрифта
						if(prop!='fontSize'){
							elem.item(i).style[prop] = css;
						// если задаем размер шрифта
						}else{
							elem.item(i).style[prop] = objButton.element.value;
						}
						// alert(elem.item(i).style[prop]+'-style');
						// alert(objthis.answers.item(i).style[prop]);
					}else{
						elem.item(i).style[prop]='';
					}
				}
				break;
